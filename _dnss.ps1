...
{	
    ...
    $result			= $cr+"  ISP    =  $result "+ $cr + "  DN     = " + $domain + $cr +  "  LOC    = " + $country +  $cr +  "  STATUS = " + $abuse
    return $result
}
...
if ($_display)
{
	...
	write-host -f red "          **DISABLED ON UMI TERM SERVERS**"
...
{
	if ($_file_)
		{if (Test-Path $_data_file -PathType Leaf){}else{write-host -f white -b fred  " ** ERROR: NO  DATAFILE LOCATED !! ** $cr    Please retype a valid filename   ";exit}}
	else
...
{
	$data = Get-Clipboard
	if (!$data){write-host  -f white -b red  " ** ERROR: NO DATA IN CLIPBOARD !! ** Please reattempt Cut & Paste ";exit}
	$data=$data.replace(" ","")
}
					...
					do
						{
							if (($result[$count].ipaddress) -like "*.*" )
							{
								...
								write-host $output
								$_output= $_output+ $result[$count].name + " - " + $result[$count].ipaddress
							}
							$count=$count + 1
						}	
