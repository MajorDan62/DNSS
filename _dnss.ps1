...
if (!$_mem)
	{if ($_file_)
		...
		$data = (Get-Content $_data_file).replace(" ","")	
}	
else	{$data = (Get-Clipboard).replace(" ","")}
					...
					if ($result.count -gt 1)
						...
						do
							{			
								$output		= $result[$count].name + " - " + $result[$count].ipaddress 
								write-host $output
								$_output	= $_output+ $result[$count].hostname + " - " + $result[$count].ipaddressfs
